...
<div class="container">
<form action="/transaksisementara" method="post">
  @csrf
<div class="row">
  <div class="col-lg-10">
      @csrf
  ...
  <div class="mb-3 row">
    ...
    <div class="col-sm-10">
      <input type="number" min="1" class="form-control  @error('harga') is-invalid @enderror" id="harga"name="harga" readonly>
      @error('harga')
  ...
  <div class="mb-3 row">
    ...
    <div class="col-sm-10">
      <input type="number" class="form-control @error('jml_beli') is-invalid @enderror" id="jml_beli" value="{{old('jml_beli')}}" name="jml_beli"required>
      @error('jml_beli')
      <div class="invalid-feedback">
        {{$message}}
      </div>
  ...
  <div class="mb-3 row">
      ...
      <div class="col-sm-10">
          <input type="number" min="0" class="form-control @error('jml_beli') is-invalid @enderror " id="jml_harga"name="jml_harga" readonly>
          @error('jml_harga')
      <div class="invalid-feedback">
        {{$message}}
      </div>
    ...
    <div class="mb-3 row justify-content-center">
      <button type="submit" class="btn btn-success btn-sm"> <i class="bi bi-cart-plus"></i> Masukan Keranjang</button>
  </div>
  </form>
...
@php
  $totalharga = 0;
@endphp
  @foreach($produksementaras as $ps )
    <tr>
      ...
      <td>{{$ps->harga}}</td>
      <td>
      <form action="/transaksisementara/{{ $ps->id }}"method="post" class="d-inline">
                                        @method('delete')
                                        @csrf
...
@if($totalharga>0)
      ...
      <h5 class="text-center mb-4">Total Tagihan : {{ formatRupiah($totalharga)}}</h5>
      <input type="hidden" class="form-control" id="total_harga"name="total_harga" value="{{ $totalharga}}">
...
@endif
  ...
  <div class="mb-3 row">
    ...
    <div class="col-sm-10">
     <input type="number" min="1" class="form-control" id="kembalian"name="kembalian" required readonly>
    </div>
  </div>
  <div class="mb-3 row justify-content-center">
    <button class="btn btn-primary btn-sm " type="submit"><i class="bi bi-cart-check"></i> Checkout</button>
...
<table class="transaksi"id="transaksi">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>{{number_format($transaksi->jml_harga)}}</td>
...
<script>
    // Deklarasi variabel array untuk menyimpan data transaksi sementara
    ...
    $(document).ready(function() {
      ...
      $('#nama_produk').change(function() {
        ...
        $('#jml_bayar').on('input', function() {
          ...
          var kembalian=parseFloat(jumlahbayar) - totalharga;
